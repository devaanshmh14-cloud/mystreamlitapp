...
st.markdown(
    '<h1 class="float-title">Physics Edexcel</h1>',
    unsafe_allow_html=True
)

st.write("Welcome! This page contains all the resources for your success in Edexcel GCSE Physics. All the resources uploaded are free to access and use. I hope you found it helpful!")
...
if st.session_state.show_past_papers:
    ...
    if st.button("June 2024 Paper 1 (Foundation)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1WwHUM62QRsrks_9IG0cOzPuttifTYtZ3/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 1 (Higher)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1MOMNdHc35f0UZJREIi0BAjXL7gGx0aeD/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 2 (Foundation)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1bYlapORVJPj_lg1-EOJzFrwPhmaUHZcn/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 2 (Higher)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1FvW2uLOEtTTUW9j-srR9Ha3GiV0B059i/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
...
if st.session_state.show_mark_schemes:
    ...
    if st.button("June 2024 Paper 1 (Foundation)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1oJb3yHzokGYu3xC87a8dYLgg71gIHfmJ/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 1 (Higher)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1ZrFDUTlDBiFrrHV_1jnyXrWV4XRUSHl3/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 2 (Foundation)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1U9oLI6WWc8HznYu_L668lPWM2uaVmpPK/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.button("June 2024 Paper 2 (Higher)", width=110000):
        pdf_url = "https://drive.google.com/file/d/1EStiP12jmvcVzA2_80QiPp9Qg9QYWE45/preview"
        st.session_state.selected_pdf = None if st.session_state.selected_pdf == pdf_url else pdf_url
    if st.session_state.selected_pdf:
        st.markdown(f'<iframe src="{st.session_state.selected_pdf}" width="100%" height="800"></iframe>', unsafe_allow_html=True)
...
if st.session_state.show_video:
    ...
    video_url = "https://youtube.com/playlist?list=PLidqqIGKox7UVC-8WC9djoeBzwxPeXph7&si=YqJISEST-NCSeLy2"
